...
const EP_SECTIONS = [
  ...
  },
  {
    ...
    fields:[
      ...
      {key:"thread",    label:"Series Thread",    icon:"🧵", color:"#6366f1", timing:"Ep link",placeholder:"Promise In: what did the last episode leave open that this episode addresses?\nPromise Out: what new promise does this episode make to keep the audience coming back?", rows:3},
    ]
  },
  {
    id:"production",
...
export default function BeatFramework() {
  ...
  const validateReg = () => {
    ...
    if (!regFields.phone.trim())                                errs.phone = "Phone is required";
    else if (!/^[\d\s\-\+\(\)]{7,}$/.test(regFields.phone))    errs.phone = "Enter a valid phone number";
    return errs;
  };
  ...
  const FEEDBACK_QUESTIONS = [
    ...
    { id:"recommend", label:"Would you recommend this to a writer?", sub:"Would you suggest ARCH to a colleague or friend?" },
  ];
  ...
  const isSaved = useSaveIndicator([
    ...
    premiseParts, premiseFreeform, projectTitle, genreRanking
  ]);
  ...
  // Auto-select the beat for the current episode
  const currentEpBeat = beatAssignments[activeEp?.ep] || 1;

  // Episode-level inputs: {genre: {epNum: {field: value}}}
  ...
  const stepComplete = {
    ...
    foundation: true, // always completeable by reading
    genre: genreRanking[0] !== null,
        ...
        <div style={{width:"100%",maxWidth:460,position:"relative",zIndex:1}}>
          ...
          <div style={{textAlign:"center",marginBottom:36}}>
            <div style={{display:"inline-flex",alignItems:"center",gap:10,marginBottom:12}}>
              ...
              <div style={{textAlign:"left"}}>
                <div style={{fontSize:7,letterSpacing:"0.4em",color:"#6366f1",textTransform:"uppercase",marginBottom:1}}>Vertical Story Intelligence</div>
                <div style={{fontSize:22,fontWeight:700,color:"#fff",letterSpacing:"0.05em"}}>THE ARCH FRAMEWORK</div>
              </div>
            </div>
          ...
          <div style={{background:"#09090f",border:"1px solid #12172a",borderRadius:10,padding:"28px 28px 24px",boxShadow:"0 20px 60px #00000060"}}>
            ...
            <div style={{marginBottom:24}}>
              <label style={{display:"block",fontSize:9,color:"#9ca3af",letterSpacing:"0.15em",textTransform:"uppercase",marginBottom:6}}>Phone Number</label>
              <input
                type="tel"
            ...
            >
              {regSubmitting ? "Setting up your workspace..." : "Enter the ARCH Framework →"}
            </button>

            <div style={{marginTop:14,fontSize:9,color:"#374151",textAlign:"center",lineHeight:1.7}}>
              Your information is used only to personalise your experience<br/>and will never be sold or shared with third parties.
            </div>
  ...
  return (
    <div style={{fontFamily:"'Georgia',serif",background:"#06080f",minHeight:"100vh",color:"#e2ddd4"}}>
      ...
      {/* ── HEADER ── */}
      <div style={{background:"linear-gradient(180deg,#0c0e1c,#06080f)",borderBottom:"1px solid #12172a",padding:"16px 30px 0"}}>

        {/* Brand row */}
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}>
          <div style={{width:3,height:36,background:`linear-gradient(180deg,#6366f1,${accent},#ffd700)`,borderRadius:2}}/>
          <div style={{flex:1}}>
            <div style={{fontSize:8,letterSpacing:"0.35em",color:"#6366f1",textTransform:"uppercase",marginBottom:1}}>Vertical Story Intelligence Platform</div>
            <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
              <h1 style={{margin:0,fontSize:17,fontWeight:700,color:"#fff",letterSpacing:"0.05em"}}>THE ARCH FRAMEWORK</h1>
              {projectTitle&&<span style={{fontSize:11,color:"#374151",fontStyle:"italic"}}>— {projectTitle}</span>}
          ...
          <div style={{display:"flex",gap:6,alignItems:"center",flexShrink:0}}>
            ...
            <div style={{display:"flex",gap:2}}>
              {SERIES_LENGTHS.map(n=>(
                <button key={n} onClick={()=>{setSeriesLength(n);setActiveEpIdx(0);}} style={{background:seriesLength===n?accent+"22":"transparent",border:`1px solid ${seriesLength===n?accent+"60":"#1e2535"}`,color:seriesLength===n?accent:"#374151",padding:"3px 8px",borderRadius:3,cursor:"pointer",fontSize:8,fontFamily:"inherit",letterSpacing:"0.1em"}}>{n}EP</button>
              ))}
            </div>
          </div>
        </div>
      ...
      <div style={{padding:"22px 30px",maxWidth:980}}>
        ...
        {activeTab==="project" && (
          <div>
            ...
            {setupStep==="foundation" && (
              <div style={{background:"#09090f",border:"1px solid #12172a",borderRadius:8,padding:"20px 22px"}}>
                <div style={{marginBottom:16}}>
                  <div style={{fontSize:9,color:"#8b5cf6",letterSpacing:"0.25em",textTransform:"uppercase",marginBottom:3}}>Step 3 — The ARCH Framework</div>
            ...
            {setupStep==="board" && (
              <div style={{background:"#09090f",border:"1px solid #12172a",borderRadius:8,padding:"20px 22px"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:10}}>
                  ...
                  <div style={{display:"flex",gap:4}}>
                    {SERIES_LENGTHS.map(n=>(
                      <button key={n} onClick={()=>{setSeriesLength(n);setActiveEpIdx(0);}} style={{background:seriesLength===n?"#3b82f6":"transparent",border:`1px solid ${seriesLength===n?"#3b82f6":"#1e2535"}`,color:seriesLength===n?"#fff":"#6b7280",padding:"4px 10px",borderRadius:4,cursor:"pointer",fontSize:9,fontFamily:"inherit"}}>{n} EP</button>
                    ))}
                  </div>
                </div>
            ...
            {/* ── CLEAR PROJECT ── */}
            <div style={{marginTop:28,paddingTop:18,borderTop:"1px solid #0d1020",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <div>
        ...
        {activeTab==="write" && projectReady && (
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"220px 1fr",gap:14,alignItems:"start"}}>
              ...
              {/* ── LEFT SIDEBAR: Episode navigator ── */}
              <div>
                    ...
                    {seriesEpisodes.map((ep,i)=>{
                      ...
                      return (
                        <button key={i} onClick={()=>{
                          ...
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{fontSize:9,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{ep.title}</div>
                  ...
                  return (
                    <div>
                        ...
                        <div style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:10}}>
                          ...
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:4,flexWrap:"wrap"}}>
                              <span style={{fontSize:12,fontWeight:700,color:"#fff",background:accent+"22",border:`1px solid ${accent}40`,padding:"2px 11px",borderRadius:10}}>EP {epNum} — {activeEp?.title}</span>
                      ...
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:14,paddingTop:14,borderTop:"1px solid #12172a"}}>
                        ...
                        <button onClick={()=>{if(safeIdx<seriesEpisodes.length-1){const ni=safeIdx+1;setActiveEpIdx(ni);setWritingBeat(beatAssignments[seriesEpisodes[ni].ep]||1);}}} disabled={safeIdx===seriesEpisodes.length-1} style={{background:safeIdx===seriesEpisodes.length-1?"transparent":accent,border:"none",color:safeIdx===seriesEpisodes.length-1?"#1e2535":"#fff",padding:"7px 14px",borderRadius:4,cursor:safeIdx===seriesEpisodes.length-1?"not-allowed":"pointer",fontSize:10,fontFamily:"inherit",fontWeight:600}}>
                          {nextEp?`EP${nextEp.ep} →`:"Series Complete"}
                        </button>
                      </div>
                    </div>
                  );
                    ...
                    return (
                      <tr key={ep.ep}>
                        ...
                        <td style={{background:rowBg,padding:"6px 10px",verticalAlign:"top",borderRight:"1px solid #12172a",minWidth:140}}>
                          <div style={{fontSize:10,color:"#e2ddd4",fontWeight:600,marginBottom:2,lineHeight:1.3}}>{ep.title}</div>
                          <div style={{fontSize:8,color:assignedBeat.color}}>{assignedBeat.icon} {assignedBeat.name}</div>
                        </td>
                        {STORY_MOMENTS.map((m,mIdx)=>{
                          const isActive = mIdx===primaryMomentIdx;
          ...
          <div>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14,flexWrap:"wrap",gap:8}}>
              <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                <span style={{fontSize:10,color:"#6366f1",fontWeight:700,letterSpacing:"0.1em",marginRight:6}}>THE ARCH FRAMEWORK</span>
        ...
        {activeTab==="mapping" && (
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 36px 1fr",alignItems:"start"}}>
              ...
              <div style={{display:"flex",justifyContent:"center"}}><div style={{width:1,minHeight:420,background:"linear-gradient(180deg,transparent,#12172a 15%,#12172a 85%,transparent)"}}/></div>
              ...
              <div>
                ...
                <div style={{marginBottom:24}}>
                  ...
                  <div style={{fontSize:11,color:"#6b7280",lineHeight:1.6}}>
                    Rate each area — your answers help us improve the ARCH Framework for every writer who uses it.
        ...
        {activeTab==="export" && (
          <div>
            <div style={{display:"flex",gap:7,marginBottom:14,flexWrap:"wrap"}}>
              {(genreRanking.filter(Boolean).length>0?genreRanking.filter(Boolean):Object.keys(genreData)).map((key,i)=>{const g=genreData[key];return<button key={key} onClick={()=>setActiveGenre(key)} style={{background:activeGenre===key?g.accentColor:"transparent",border:`1px solid ${activeGenre===key?"transparent":"#12172a"}`,color:activeGenre===key?"#fff":"#4b5563",padding:"6px 15px",borderRadius:4,cursor:"pointer",fontSize:10,letterSpacing:"0.12em",textTransform:"uppercase",fontFamily:"inherit",transition:"all 0.2s"}}>{GENRE_RANK_ICONS[i]||""} {g.icon} {g.label}</button>;})}
            </div>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:14}}>{SERIES_LENGTHS.map(n=><button key={n} onClick={()=>setSeriesLength(n)} style={{background:seriesLength===n?accent:"transparent",border:`1px solid ${seriesLength===n?accent:"#1e2535"}`,color:seriesLength===n?"#fff":"#6b7280",padding:"4px 11px",borderRadius:4,cursor:"pointer",fontSize:10,fontFamily:"inherit",transition:"all 0.18s"}}>{n} EP</button>)}</div>
            <p style={{color:"#6b7280",fontSize:11,lineHeight:1.6,marginBottom:14}}>Exporting: <strong style={{color:"#e2ddd4"}}>{projectTitle||"Untitled"}</strong> — {genreRanking.filter(Boolean).map((g,i)=><span key={g} style={{color:genreData[g].accentColor}}>{GENRE_RANK_ICONS[i]} {genreData[g].label} </span>)} · {seriesLength} episodes</p>
            ...
            {[
              {name:"Series Story Board",  ext:".xlsx",    desc:`${seriesLength}-episode board · 12 structural moments · All written cells included · Moment reference sheet.`,status:"Ready",color:"#22c55e"},
              {name:"Fountain Format",    ext:".fountain",desc:"Premise, treatment, all beats. Imports into Arc Studio Pro, Highland 2, Fade In, and 12+ apps.",status:"Ready",color:"#4a9eff"},
              {name:"Final Draft",        ext:".fdx",     desc:"Industry standard. Premise, treatment, beat markers, scene descriptions, and dialogue.",status:"Ready",color:"#ffd700"},
              {name:"Full Project PDF",   ext:".pdf",     desc:"Title page, premise, treatment, beat map, series arc graph. Complete client-ready document.",status:"Ready",color:"#00ffaa"},
              {name:"Treatment PDF",      ext:".pdf",     desc:"Treatment only — formatted for industry submission.",status:"Ready",color:"#ff4aff"},
              {name:"JSON / API",         ext:".json",    desc:"Complete project data for custom workflows and integrations.",status:"Pro",color:"#ff8c00"},
              {name:"CSV Arc Data",       ext:".csv",     desc:`Tension arc data for all ${seriesLength} episodes.`,status:"Pro",color:"#a855f7"},
            ].map((fmt,i)=>(
              <div key={i} style={{border:"1px solid #12172a",borderLeft:`3px solid ${fmt.color}`,borderRadius:5,padding:"11px 13px",marginBottom:6,display:"flex",alignItems:"center",gap:11,background:"#0a0c14"}}>
                ...
                <div style={{flex:1}}><div style={{color:"#e2ddd4",fontSize:12,marginBottom:1}}>{fmt.name}</div><div style={{color:"#6b7280",fontSize:10}}>{fmt.desc}</div></div>
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  <span style={{fontSize:8,color:fmt.status==="Pro"?"#ff8c00":"#00ffaa",background:fmt.status==="Pro"?"#ff8c0012":"#00ffaa12",border:`1px solid ${fmt.status==="Pro"?"#ff8c0035":"#00ffaa35"}`,padding:"2px 5px",borderRadius:3}}>{fmt.status}</span>
                  <button style={{background:fmt.color,border:"none",color:"#000",padding:"5px 12px",borderRadius:4,cursor:"pointer",fontSize:9,fontWeight:700,fontFamily:"inherit"}}>Export</button>
                </div>
              </div>
            ))}
